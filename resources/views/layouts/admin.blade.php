...
<body class="bg-slate-900 font-sans antialiased text-slate-200" x-data="{ sidebarOpen: false }">

    <div class="flex min-h-screen">
        
        <div class="fixed inset-0 bg-slate-950/60 z-20 md:hidden transition-opacity" 
             x-show="sidebarOpen" 
        ...
        <aside class="w-64 bg-slate-950 border-r border-slate-800 flex flex-col fixed inset-y-0 left-0 z-30 transform md:transform-none transition-transform duration-300 ease-in-out"
            ...
            <div class="p-4 border-t border-slate-800 bg-slate-955">
                <div class="flex items-center justify-between">
                    ...
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit" class="text-slate-500 hover:text-rose-450 transition-colors p-1 rounded-lg hover:bg-slate-900">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="flex-1 md:pl-64 flex flex-col min-w-0">
            <header class="h-16 bg-slate-950/50 backdrop-blur-md border-b border-slate-800 flex items-center justify-between px-6 md:px-8 sticky top-0 z-10">
                <div class="flex items-center gap-4">
                    <button @click="sidebarOpen = true" class="text-slate-400 hover:text-white md:hidden p-1 rounded-lg hover:bg-slate-900">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16" />
                        </svg>
                    </button>
                    <h2 class="text-sm font-semibold text-slate-400">Overview</h2>
                </div>
            ...
            <main class="flex-1 p-4 md:p-8 bg-slate-900">
                @yield('content')
            </main>
        </div>
    </div>
